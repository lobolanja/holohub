...
#
# Container-based RX launcher for Mixed-Transport end-to-end testing — ANO path.
# Runs the connext_app with ANO (GPU Direct / DPDK) only; DDS is disabled.
# This is RX1 in the mixed-transport topology, validating ANO transport isolation.
#
# Prerequisites:
#   - Build completed: ./holohub build connext_app_cpp
#   - Physical RX NIC available with PCIe address
...
echo "=================================================="
echo "Connext Mixed-Transport E2E Test - RX1 (ANO)"
echo "  Transport: ANO (GPU Direct) only"
echo "=================================================="
...
echo "=================================================="
echo ""
echo "Starting RX1 (ANO) application in container..."
...
# Create unique DPDK runtime directory for mixed-transport ANO RX
mkdir -p /home/$USER/dpdk_rx_mixed
chmod 777 /home/$USER/dpdk_rx_mixed

# Run RX application in container with DPDK privileges (needed for ANO path)
...
./holohub run-container connext_app_cpp \
  ...
  -- /bin/bash -c "echo '=== Starting connext_app Mixed-Transport RX1 (ANO) ===' && $PATH_TO_APP/connext_app $PATH_TO_APP/tests/config/test_mixed_rx_ano.yaml 2>&1 || echo 'Exit code:' \$?"
