...
#
# Container-based TX application launcher for ANO end-to-end testing
# This script runs the connext_sender_receiver app in TX mode in a Docker container
#
# Prerequisites:
...
echo "=================================================="
echo "Connext ANO E2E Test - TX Container"
echo "=================================================="
...
# Create separate DPDK runtime directory for TX to avoid conflicts with RX
mkdir -p /home/$USER/dpdk_tx
chmod 777 /home/$USER/dpdk_tx
...
./holohub run-container connext_app_cpp \
  ...
  -- /bin/bash -c "echo '=== Starting connext_app with 30s timeout ===' && timeout 30s $PATH_TO_APP/connext_app $PATH_TO_APP/tests/config/test_ano_tx.yaml 2>&1 || echo 'Exit code:' \$?"