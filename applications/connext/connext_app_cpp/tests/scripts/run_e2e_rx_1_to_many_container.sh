...
#
# Container-based RX application launcher for ANO 1-to-Many end-to-end testing
# This script runs the connext_app in RX mode for a specific subscriber
#
# Usage:
#   ./run_e2e_rx_1_to_many_container.sh <subscriber_id>
#   where subscriber_id is 1, 2, or 3
#
# Prerequisites:
...
CONFIG_FILE="config/test_ano_rx_1_to_many_sub${SUBSCRIBER_ID}.yaml"
...
echo "=================================================="
echo "Connext ANO 1-to-Many E2E Test - RX Container"
...
echo "UDP Port:    500${SUBSCRIBER_ID}"
...
mkdir -p /home/$USER/dpdk_rx_sub${SUBSCRIBER_ID}
chmod 777 /home/$USER/dpdk_rx_sub${SUBSCRIBER_ID}
...
./holohub run-container connext_app_cpp \
  ...
  -- /bin/bash -c "echo '=== Starting connext_app RX Subscriber ${SUBSCRIBER_ID} ===' && $PATH_TO_APP/connext_app $PATH_TO_APP/tests/config/test_ano_rx_1_to_many_sub${SUBSCRIBER_ID}.yaml 2>&1 || echo 'Exit code:' \$?"
